...
Autonomous robots have found increasing use in numerous devices, from helping customers navigate stores \citep{mcsweeney_2018} to keeping floors clean \citep{sanfacon_2017} and mowing lawns \citep{udelhofen_2018}. A common challenge with such systems is keeping the robot in bounds. This commonly means for the robot to be aware where on a two-dimensional map it is currently located. In certain applications however, "in bounds" rather involves staying on a set of allowed surface types, such as autonomous robot lawn mowers staying only on areas covered in grass. In such devices one may be content with knowing that the robot roams around remaining on its designated surface type rather than having knowledge of its exact position, and a surface classification method of great use. 
...
In this work we wish to distinguish grass surfaces from non-grass surfaces through the use of millimeter-wave radar. The model should work for a robot moving forward at a constant pace. The application most closely resembling this objective is perhaps road condition monitoring, where one attempts to determine the state of a road for safety and driver-assistance purposes. In \citep{finkele_schreck_wanielik_1995} an array of radar transmitters with different polarities were used sequentially to illuminate the same surface area. Seven receivers, also with different polarizations, was then collected for every area type. Other bistatic setups, that is with transmitter and receiver spatially separated, for road condition monitoring were considered in \citep{kees_detlefsen_1994} and \citep{finkele_1997}. Monostatic exeperiments, with transmitting and receiving antenna at the same location, have found varying degrees of success. In \citep{viikari_varpula_kantanen_2008} and \citep{häkli_säily_koivisto_huhtinen_dufva_rautiainen_toivanen_nummila_2013} a monostatic setup was used to compare returning strengths of linear (E- and H-plane) polarizations. 

These polarization based methods are however founded on measuring asphalt with or without a thin layer of ice or water. In the present case of measuring grass, we are instead more interested in a surfaces topography not captured by examining slight differences in polarization. 


% Next: Motivate the use of temporal (slow-time) methods

%THESE ARE ALL POLARIZATION BASED
...
%(2) Polarimetric \citep{finkele_schreck_wanielik_1995}. Used array of transmitters with different polarities and and array of receivers receiving different polarities. Some different classifiers. 

%(3) Polarimetric \citep{kees_detlefsen_1994}. Bistatic setup, estimated polarization matrix. 

%(4) Polarimetric \citep{finkele_1997}. Ice detection, bistatic setup. Polarization measurements.

%(5) Uninteresting - 0.5 GHzi

%(6) Polarimetric - Monostatic - \citep{viikari_varpula_kantanen_2008} - Road surface recognition monostatic. Estimating backscattering with different kinds of polarizations.

%(6b) 

%(7)

%(8) Polrimetric - Monostatic - \citep{häkli_säily_koivisto_huhtinen_dufva_rautiainen_toivanen_nummila_2013} - Road surface recognition monostatic, estimating backscatterign
