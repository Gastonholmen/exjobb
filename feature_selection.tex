...
As discussed in section \ref{destrait}, it is reasonable to believe that the envelope shape from a flat and a rough surface may differ. Considering our goal is to differentiate grass - which is without doubt a rugged surface - from other surfaces, using the shape of radar sweeps is motivated. However, as mentioned in section \ref{multsweep} we ought to improve the signal quality by averaging over a number of sweeps as in figure \ref{fig:sweep_average}. In addition, this gives the samples a higher resemlance.  
...
\begin{figure}[h]
	...
	\caption{By averaging a number of consecutive sweeps, we reduce noise and make samples more similar. The dashed line shows the average of the solid envelopes. Note that the actual averaging is done over complex IQ data, wheras this figure depicts absolute values. }
