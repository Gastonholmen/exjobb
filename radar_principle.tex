...
\begin{equation}
	m(\tau) = \int x_T(t + \tau)r(t) dt
\end{equation}

where $x_T(t)$ and $r(t)$ are the transmitted and received pulses respectively. If only a single point scatterer is in the target scene and $A(t)$ is a rectangular window, $m(\tau)$ will be on the form 
