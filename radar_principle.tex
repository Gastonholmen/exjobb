...
which means that the distance can be calculated from the phase shift through
...
\subsection{Power dissapation and radar cross section}
...
If a scatterer is present at range $d$ with target cross section $\sigma$ and gain $G_t$, the fraction of the radar power reflected by the target is given by 
...
Finally, only a portion $P_r = P_eA_r$ of the power is captured by the receiver, where $A_r$ is a function of transmission wavelength and receive antenna gain. Putting this all together, we obtain the \emph{radar range equation}
...
The radar range equation clearly states that power dissapates rapidly, by a factor $d^4$, with range. Thus, increasing the distance to a target object with a factor 2 will return only $1/16$ of the power otherwise received. 
