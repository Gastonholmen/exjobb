...
After a radar wavelet is transmitted and reflected back to the radar from a single scatterer, it can be described mathematically as \citep{richards_2014}
\begin{equation}
	\label{eq:raw_sweep}
	\textrm{rx}(t)=A(t)\sin(\Omega t +\theta (t)),
\end{equation}
...
\begin{figure}[h]
	...
	\caption{A single radar sweep which has been measured from 7 to 23 cm. The largest amplitude between at around 18 cm. suggests there is some object present at that distance from the sensor.}
...
\begin{itemize}
	...
	\item delay/distance
	\item f=60GHz
	\item derive $\lambda$ from 60GHz frequency
	\item 
\end{itemize}
