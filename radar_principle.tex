...
Radars are typically active systems, meaning that they have a radiating antenna and do not depend on ambient radiation \citep{skolnik_2009}. To perform measurements, a wavelet pulse $x_T(t)$ with some carrier angular frequency $\Omega$ and envelope $A(t)$ is transmitted towards an object of interest. If a perfect scatterer is located some distance away from the radar transmitter, and a radar pulse with envelope $A(t)$ and angular frequency $\Omega$ is transmitted towards it, the returning signal will have the form \citep{richards_2014}

\begin{equation}
	r(t) = CA(t)\sin(\Omega_r t + \theta(t))
\end{equation}

where $C$ is some constant related to the dissapation of power throughout space and some radar system properties. If the scatterer was present a distance $d$ from the scatterer, the wavelet will return $2d/c$ seconds after transmission, where $c$ denotes the speed of light. Thus the distance to the target can be concluded to be  
...
Note that we must divide by 2 in equation \ref{eq:dist_time} in order to accomodate for the pulse travelling back and forth (or, perhaps more accurately, forth and back) between the sensor and the scatterer. This delay will appear in $\theta(t)$ as a phase-shift. 


% Here: Talk about the two frequencies appearing! The doppler one and the "doppler" one. 

% Have a part on bandwidth

% Have a part about radar cross section
