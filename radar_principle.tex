...
To understand what information can be obtained from such a one dimensional signal, a single scattering object some distance away from the transmitter is here considered. If a radar pulse with envelope $A(t)$ and carrier frequency, $f_c$, is transmitted towards it, the returning signal will be a delayed pulse on the form \citep{richards_2014}
...
By measuring the delay $B$ in \eqref{eq:returning} we can thus simply calculate the distance through
...
Then, given that the scattering process also is isotropic and lossless the power reflected back at the radar receiver is
...
In the preceding section it was shown that the scattering response carry information about target range, reflectivity and radial velocity. This section explains how the radar captures the returning signals to generate raw radar data through a \emph{matched filter} approach.

A matched filter approach means convolving the returning signal with a copy of the transmitted signal. Since the radio channel is assumed to be linear and time invariant (LTI) the received signal consists of a sum of weighted and time shifted transmitted signals. It involves generating an internal pulse, called an analysis pulse, as a delayed copy of the transmitted pulse. The received signal and the generated signal are then multiplied and summed to form one measurement point $m(\tau)$, where $\tau$ is the internal delay of the analysis pulse. 
...
\begin{figure}[t]
	...
	\caption{A single raw data signal formed by matched filtering has been measured from 7 to 23 cm. The largest amplitude at around 18 cm. suggests there is some object present at that distance from the sensor.}
