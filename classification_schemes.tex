...
Logistic regression (\gls{lr}) is similar to linear regression, in which the parameters $\{b_i\}_{i=0}^n$ are optimized to model the relationship between the inputs $\{X_i\}$ and the continuous output $Y$
\begin{equation}
...
%\gls{lr} also differs from linear regression in its cost function. While the parameters in linear regression are usually estimated by 

For further details about \gls{lr}, including how its parameters are optimized, we refer to \citep{a_smola_svn_vishwanathan_2010}.
